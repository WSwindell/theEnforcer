...
public class HardeningFormula extends AbstractDynamicCard {

    // TEXT DECLARATION

    public static final String ID = DefaultMod.makeID(HardeningFormula.class.getSimpleName());
    ...
    // /STAT DECLARATION/
    public HardeningFormula() {
        this(true);
    }

    public HardeningFormula(boolean makePreview) {
        ...
        if(makePreview) {
            cardsToPreview = new TemperingFlask(false);
        }
    }
    ...
    public void triggerOnExhaust() {
        if (upgraded) {
            AbstractCard newCard = (new TemperingFlask()).makeCopy();
            newCard.upgrade();
            AbstractDungeon.actionManager.addToBottom(
            ...
            AbstractDungeon.actionManager.addToBottom(
                    new MakeTempCardInHandAction(new TemperingFlask())
            );
        }
    }
