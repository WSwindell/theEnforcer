...
public class Shatter extends AbstractDynamicCard {
    ...
    public static final String ID = DefaultMod.makeID(Shatter.class.getSimpleName());
    ...
    public Shatter(boolean makePreview) {
        super(ID, IMG, COST, TYPE, COLOR, RARITY, TARGET);
        tags.add(EnforcerTags.CAN_FLIP);
        if(makePreview) {
            cardsToPreview = new CollectScraps(false);
        }
    }
    ...
    public void triggerOnExhaust() {
        if (upgraded) {
            AbstractCard newCard = (new CollectScraps()).makeCopy();
            newCard.upgrade();
            AbstractDungeon.actionManager.addToBottom(
            ...
            AbstractDungeon.actionManager.addToBottom(
                    new MakeTempCardInHandAction(new CollectScraps())
            );
        }
    }
